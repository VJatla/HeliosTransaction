function  img_alg1 = henney_harvey(synoptic_path,photomap_path)
%Flags
...
for row = 181:360
    for col = 1:720
        if(largeimage(row,col) < (mean_image_data - stdev_image_data*(0.7 + 0.1*cos(deg/radeg))))
...
for row = 1:180
    for col = 1:720
        if(largeimage(offset - row,col) < (mean_image_data - stdev_image_data*(0.7 + 0.1*cos(deg/radeg))))
...
if use_original_open_close
    ...
    % This code is from trying to optimize henney harvey method.
